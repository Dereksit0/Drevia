...
import { RiDoubleQuotesL, RiInstagramLine } from "react-icons/ri";
...
function VideoCard({ video, index }: { video: typeof videos[0]; index: number }) {
  return (
    ...
    >
      <figure
        className="relative rounded-2xl overflow-hidden border border-white/10 bg-[#0c0c0c] group"
      ...
      >
        ...
        {/* Decorative quote mark — center */}
        <div className="absolute inset-0 flex items-center justify-center z-0" aria-hidden="true">
          <RiDoubleQuotesL size={72} className="text-white/[0.06]" />
        </div>

        {/* Bottom gradient + info */}
        <figcaption className="absolute bottom-0 left-0 right-0 z-10 bg-gradient-to-t from-black via-black/80 to-transparent pt-16 pb-5 px-5">
          <p className="text-white/75 text-xs leading-relaxed mb-4 line-clamp-4 italic">
            {video.quote}
          </p>
          <div className="flex items-center gap-2.5">
            ...
            <div>
              ...
              <p className="text-white/50 text-[10px] mt-0.5">{video.role} · {video.company}</p>
            </div>
          </div>
        </figcaption>
      </figure>
    ...
    <section className="bg-white dark:bg-black py-24 lg:py-32 overflow-hidden border-t border-black/6 dark:border-white/6">
        ...
        >
          ...
          <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
            <h2 className="text-3xl sm:text-4xl lg:text-5xl font-bold text-black dark:text-white tracking-tight max-w-xl">
              Lo que opinan sobre
              <br />
              <span className="text-black/45 dark:text-white/50">nuestro trabajo</span>
            </h2>
            <div className="flex items-center gap-3 flex-shrink-0">
              <a
                ...
                className="inline-flex items-center gap-1.5 text-black/55 dark:text-white/55 hover:text-black dark:hover:text-white text-xs font-medium transition-colors duration-200"
              >
          ...
          <p className="mt-3 text-black/55 dark:text-white/55 text-sm sm:text-base max-w-lg leading-relaxed">
      ...
      >
        <div className="flex gap-1.5">
          {videos.map((_, i) => (
            <span key={i} className={`h-0.5 rounded-full transition-all duration-300 ${i === 0 ? "w-5 bg-black/60 dark:bg-white/60" : "w-2 bg-black/15 dark:bg-white/15"}`} />
          ))}
        </div>
        <span className="text-black/40 dark:text-white/40 text-xs">Desliza para ver más</span>
      </motion.div>
    </section>
  );
}
